...
CREATE TABLE IF NOT EXISTS `keypanel`.`entities` (
  ...
  `type` ENUM('SUPERADMIN','ADMIN','MEMBER','PROFILE') NULL DEFAULT 'PROFILE' COMMENT 'Tipo de usuario',
  ...
  `sector` ENUM('','PUBLIC','PRIVATE') NULL DEFAULT '' COMMENT 'propiedad que permite identificar a una empresa si pertenece al sector público o privado',
...
CREATE TABLE IF NOT EXISTS `keypanel`.`entities_address` (
  ...
  `type` ENUM('BUSINESS','POBOX') NULL DEFAULT 'BUSINESS',
...
CREATE TABLE IF NOT EXISTS `keypanel`.`entities_contacts` (
  ...
  `phone_personal` CHAR(15) NULL DEFAULT NULL,
  `phone_business` CHAR(15) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `keypanel`.`entities_access_log` (
  ...
  `browser` CHAR(50) NOT NULL COMMENT 'Se registrarán 10 logins por usuario, cuando el tamaño de acceso llegue al limite, se sobreescribirá el mas antiguo.',
  PRIMARY KEY (`id`),
  INDEX `id_entity` (`id_entity` ASC),
  INDEX `date` (`date` ASC),
...
CREATE TABLE IF NOT EXISTS `keypanel`.`entities_history` (
  ...
  CONSTRAINT `fk_entities_history_entities1`
    FOREIGN KEY (`id_member`)
    REFERENCES `keypanel`.`entities` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_entities_history_entities2`
