...
CREATE TABLE IF NOT EXISTS `keypanel`.`entities` (
  ...
  `sector` ENUM('','PUBLIC','PRIVATE') CHARACTER SET 'latin1' COLLATE 'latin1_spanish_ci' NULL DEFAULT '' COMMENT 'propiedad que permite identificar a una empresa si pertenece al sector público o privado',
...
-- -----------------------------------------------------
-- Table `keypanel`.`organization_category`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `keypanel`.`organization_category` (
  `id` MEDIUMINT(6) NOT NULL AUTO_INCREMENT,
  `id_parent` MEDIUMINT(6) NOT NULL,
